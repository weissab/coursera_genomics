...
def open_reading_frames(frame, seqs):
    ...
    seqs_ORF_len = {}
    k = 0
    def find_one_frame(frame, sequence, seqs_ORF_len, k):
        ...
        if len(next_chunk) > 0:
            next_chunk, seqs_ORF_len, k = find_one_frame(0, next_chunk, seqs_ORF_len, k)
...
def main():
    ...
    seqs = create_seqs(f)
    f.close()
    seqs_ORF_len = open_reading_frames(frame, seqs)
