...
def find_reading_frame(frame, sequence, key, seqs_ORF_len, k):
    start_codons = ['ATG']
...
def open_reading_frame(frame, seqs):
    ...
    for key, sequence in seqs.items():
        next_chunk, seqs_ORF_len, k = find_reading_frame(frame, sequence, key, seqs_ORF_len, k)
        if len(next_chunk) > 0:
            next_chunk, seqs_ORF_len, k = find_reading_frame(0, next_chunk, key, seqs_ORF_len, k)
...
def main():
    ...
    seqs = create_seqs(f)
    f.close()
    seqs_ORF_len_final = open_reading_frame(frame, seqs)
    print("The tupple containing open reading frames: %s " % pretty_print(seqs_ORF_len_final))
