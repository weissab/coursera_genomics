...
def boyer_moore(p, p_bm, t):
    ...
    while i < len(t) - len(p) + 1:
        ...
        i += shift
    return occurrences, alignmnets_tried, character_comparison_total 
...
def main():
    #t, pattern = parse_args(sys.argv)
    #reference_genome = read_genome(filename)
    p = 'word'
    t = 'there would have been a time for such a word'
    lowercase_alphabet = 'abcdefghijklmnopqrstuvwxyz'
    p_bm = BoyerMoore(p, lowercase_alphabet)
    occurences, character_comparison_total, alignmnets_tried = boyer_moore(p, p_bm, t)
